...
for order in all_orders:
    ...
    if date_confirmed_from <= date_confirmed < date_confirmed_to and order['invoice_country_code'] == 'CZ':
...
if filtered_orders:
    ...
    for income in net_incomes:
        print(f"Order ID: {income['order_id']}, Net Income: {income['net_income']:.2f} CZK")

    print(f"Total Net Income: {total_net_income:.2f} CZK")
