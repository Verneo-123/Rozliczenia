...
def get_orders(date_confirmed_from, order_source, order_source_id_filter,
...
def calculate_net_income(orders, purchase_prices):
    total_net_income = 0
    net_incomes = []
    ...
    for order in orders:
        ...
        for product in order["products"]:
            product_price_net = product["price_brutto"] / (
                1 + product["tax_rate"] / 100)
            purchase_price = purchase_prices.get(str(product["product_id"]), 0) / (
                1 + product["tax_rate"] / 100)
            net_income += (product_price_net -
                           purchase_price) * product["quantity"]
...
for order_status_id_filter in order_status_id_filters:
    ...
    while True:
        orders_response = get_orders(last_date_confirmed, order_source,
                                     order_source_id_filter,
                                     order_status_id_filter)
...
for order in all_orders:
    ...
    if date_confirmed_from <= date_confirmed < date_confirmed_to and order[
            'invoice_country_code'] == 'PL':
        filtered_orders.append(order)

if filtered_orders:
    product_ids = list(
        set(product['product_id'] for order in filtered_orders
            for product in order['products']))
    inventory_id = "58444"
    ...
    purchase_prices = {
        str(pid): pdata['average_cost']
        for pid, pdata in inventory_products.items()
    }

    net_incomes, total_net_income = calculate_net_income(
        filtered_orders, purchase_prices)

    print("\nPLN Market:")
    for income in net_incomes:
        print(
            f"Order ID: {income['order_id']}, Net Income: {income['net_income']:.2f} PLN"
        )

    print(f"Total Net Income: {total_net_income:.2f} PLN")
